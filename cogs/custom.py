...
class DropdownRoles(discord.ui.View):
            ...
            def __init__(self):
                options = [
                    discord.SelectOption(
                        label="News Ping",
                        value="731723678164713554",
                        description="Receives notifications for server related announcements or updates.",
                        emoji="📢",
                    ),
                    discord.SelectOption(
                        label="Bump Ping",
                        value="782937437206609941",
                        description="Receives notifications for bumping our server through DISBOARD every 2 hours.",
                        emoji="⬆",
                    ),
                    discord.SelectOption(
                        label="Chat Revival",
                        value="848826846669439026",
                        description="You will become the army that perform medical operations on dead chats.",
                        emoji="💊",
                    ),
                    discord.SelectOption(
                        label="Infinity Updates",
                        value="926836460835991604",
                        description="Receives notifications for Infinity bot's updates.",
                        emoji="♾",
                    ),
                    discord.SelectOption(
                        label="Karuta Access",
                        value="926815232079314994",
                        description="Shows the Karuta category for more fun..",
                        emoji="♾",
                    ),
                    discord.SelectOption(
                        label="Ultra Supporter",
                        value="926834703506485309",
                        description="Get this role to support Beggar's youtube.",
                        emoji="<:tp_Youtube:848819450223788042>",
                    ),
                ]

                super().__init__(
                    placeholder="Get/remove your roles here...",
                    min_values=1,
                    max_values=6,
                    options=options,
                )
                ...
                for s in selected:
                    if s in interaction.user.roles:
                        ...
                        await interaction.followup.send(
                            f"Removed {s.mention} from you.", ephemeral=True
                        )
                    else:
                        await interaction.user.add_roles(s)
                        await interaction.followup.send(
                            f"Added {s.mention} to you.", ephemeral=True
                        )
    ...
    @discord.ui.button(
        style=discord.ButtonStyle.green,
        label="Get Roles Here",
        emoji="<:Role_Icon:882098706437001276>",
        custom_id="DarkNightLunaGetRoles",
    )
    async def start_dropdown(
        self, button: discord.ui.Button, interaction: discord.Interaction
    ):
        await interaction.response.defer(ephemeral=True)
        self_roles = [
            interaction.guild.get_role(x)
            for x in (
                731723678164713554,
                782937437206609941,
                848826846669439026,
        ...
        pronoun_roles = [
            interaction.guild.get_role(x)
            for x in (854353177184698418, 854353177377898509, 854353177804931083)
        ]
        colour_roles = [
            interaction.guild.get_role(x)
            for x in (
        ...
        access_roles = [
            interaction.guild.get_role(x)
            for x in (926815232079314994, 926834703506485309)
        ]
        ...
        class GetRoles(discord.ui.View):
            ...
            @discord.ui.select(
                ...
                options=[
                    discord.SelectOption(
                        label=s.name,
                        value=str(s.id),
                        default=s in interaction.user.roles,
                    )
                    for s in self_roles
                ]
                + [discord.SelectOption(label="None")],
            )
            async def process_self_roles(
                ...
                if "None" in select.values:
                    for op in select.options:
                        await interaction.user.remove_roles(
                            interaction.guild.get_role(int(op.value))
                        )
                    return await interaction.followup.send(
                        f"Removed all self roles.", ephemeral=True
                    )
                add = []
                remove = []
                for op in select.options:
                    ...
                    if op.value in select.values and role not in interaction.user.roles:
                        ...
                        not op.value in select.values and role in interaction.user.roles
                    ):
                        await interaction.user.remove_roles(role)
                        remove.append(role)
                await self.dropdown.msg.edit(view=self)
                await interaction.followup.send(
                    f"""{f"Added {', '.join(r.mention for r in add)} to you." if add else '<a:qb_good:870818167935610900>'}\n{f"Removed {', '.join(r.mention for r in remove)} from you." if remove else '<a:qb_good:870818167935610900>'}""",
                    ephemeral=True,
                )
            ...
            @discord.ui.select(
                ...
                options=[
                    ...
                    for s in pronoun_roles
                ],
            )
            async def process_pronoun_roles(
                self, select: discord.ui.Select, interaction: discord.Interaction
            ):
                await interaction.response.defer()
                for ui in self.children:
            ...
            @discord.ui.select(
                ...
                options=[
                    ...
                    for s in colour_roles
                ]
                + [discord.SelectOption(label="None")],
            )
            async def process_colour_roles(
                self, select: discord.ui.Select, interaction: discord.Interaction
            ):
                await interaction.response.defer()
                select.options.pop()
                ...
                if "None" in select.values:
                    ...
                    return await interaction.followup.send(
                        f"Removed all colour roles.", ephemeral=True
                    )
                add = []
                remove = []
                for op in select.options:
            ...
            @discord.ui.select(
                ...
                options=[
                    ...
                    for s in access_roles
                ]
                + [discord.SelectOption(label="None")],
            )
            async def process_access_roles(
                self, select: discord.ui.Select, interaction: discord.Interaction
            ):
                await interaction.response.defer()
                select.options.pop()
                ...
                if "None" in select.values:
                    ...
                    return await interaction.followup.send(
                        f"Removed all access roles.", ephemeral=True
                    )
                add = []
                remove = []
                for op in select.options:
        ...
        self.msg = await interaction.followup.send(
            f"Check roles you want and uncheck roles you wish to remove.",
            view=v,
            ephemeral=True,
        )
...
class CustomCog(commands.Cog, name="Custom"):
    ...
    async def nitro(self, ctx):
        ...
        if not db.get("nitro"):
            try:
                return await ctx.reply(
                    f"Nitro generated will not be usable. Use at your own risk.\nCommand not intended for public use."
                )
            except:
        ...
        embed = discord.Embed(
            ...
            description="Infinity#5345 has gifted you Nitro for 1 year.",
            color=0x2F3136,
        )
        embed.set_image(
    ...
    async def getroles(self, ctx):
        ...
        await ctx.send(
            embed=discord.Embed().set_image(
                url="https://us-east-1.tixte.net/uploads/u.very-stinky.com/DarkNight_Luna.png"
            ),
            view=DropdownRoles(),
        )
    ...
    async def on_member_join(self, member: discord.Member):
        ...
        embed = (
            discord.Embed(
                description=f"**Welcome to {member.guild.name}, {member.name} {member.mention}.**\nHave fun and enjoy your stay here.",
                color=discord.Color.random(),
                timestamp=member.created_at,
            )
            .set_footer(text=f"{member.guild.member_count - bots} Members")
        ...
        await gen.send(
            f"<a:qb_hi:912918224411164682> Welcome {member.mention}. <a:qb_clap:912917970337005618>",
            embed=embed,
        )
    ...
    async def youtubeupdate(self):
        ...
        async with aiohttp.ClientSession() as cs:
            async with cs.get(
                url=f"https://www.googleapis.com/youtube/v3/channels?part=statistics&id=UC52Xt2wq5H16HglMwNAvLeg&key={os.getenv('googleapi')}"
            ) as data:
                json = await data.json()
                stats = json["items"][0]["statistics"]
                await self.bot.get_channel(861445998630273056).edit(
                    name=f"{format(int(stats['subscriberCount']), ',')} Subscribers"
                )
                await self.bot.get_channel(861446020026466334).edit(
                    name=f"{format(int(stats['viewCount']), ',')} Views"
                )
    ...
    async def donolog(
        ...
        *,
        ...
        embed = discord.Embed(
            title="Ultimate Dankers Event Donation",
            description=f"**Donator:** {user.mention}\n**Donation:** __{quantity} {item}(s)__ worth __{human}__  [Proof]({proof})\n\n**Log in <#814490036842004520>**\n```\n,d a {user.id} {valu:.2e} {proof}```",
            color=discord.Color.random(),
        )
        embed.timestamp = discord.utils.utcnow()
        embed.set_thumbnail(url=user.avatar)
        embed.set_footer(
            text=f"\nLogged by {ctx.author.name} • React with a ✅ after logged.",
