...
class InfoCog(commands.Cog, name="Info"):
    ...
    async def links(self, ctx):
        ...
        embed.add_field(
            name="Owner",
            value=f"<a:crown:902048071343538176> {' '.join([f'<@{owner}>' for owner in self.bot.owners])}",
        )
        embed.add_field(
            name="Ping",
            value=f"<:ping:901051623416152095> {round (self.bot.latency * 1000)}ms ",
        )
        d = discord.utils.utcnow() - self.bot.startuptime
        embed.add_field(
            name="Uptime",
            value=f"<a:timer:890234490100793404> {round(d.seconds/60/60,2)} hours",
        )
        embed.add_field(
            name="Connected",
            value=f"<:discovery:894391371635499048> To **{format(len(self.bot.users),',')}** members in **{len(self.bot.guilds)}** guilds.",
            inline=False,
        )
        v = discord.ui.View()
        v.add_item(
            discord.ui.Button(
                emoji="🤖",
                label="Invite",
                url=f"{discord.utils.oauth_url(ctx.bot.application_id, permissions=discord.Permissions.none(), scopes=('bot','applications.commands'))}",
            )
        )
        v.add_item(
            discord.ui.Button(
                emoji="👾",
                label="Invite (All perms)",
                url=f"{discord.utils.oauth_url(ctx.bot.application_id, permissions=discord.Permissions.all(), scopes=('bot','applications.commands'))}",
            )
        )
        v.add_item(
            discord.ui.Button(
                emoji="<:tp_panda:839699254951804948>",
                label="Support",
                url="https://discord.gg/dHGqUZNqCu",
            )
        )
        v.add_item(
            discord.ui.Button(
    ...
    async def vote(self, ctx):
        ...
        bl = {
            ...
            "DBL": {
                "w": "https://discordbotlist.com/bots/infinity-5345/upvote",
                "v": None,
            },
            "BladeBL": {"w": "https://bladelist.gg/bots/732917262297595925", "v": None},
            "VoidBots": {
                "w": "https://voidbots.net/bot/732917262297595925/vote",
                "v": None,
            },
            "ListCord": {"w": "https://listcord.gg/bot/732917262297595925", "v": None},
            "BotLists": {"w": "https://botlists.com/bot/732917262297595925", "v": None},
            "Fateslist": {
                "w": "https://fateslist.xyz/bot/732917262297595925/vote",
                "v": None,
            },
            "Blist": {"w": "https://blist.xyz/bot/732917262297595925", "v": None},
            "MotionList": {
                "w": "https://www.motiondevelopment.top/bots/732917262297595925/vote",
                "v": None,
            },
            "DiscordServices": {
                "w": "https://discordservices.net/bot/732917262297595925",
                "v": None,
            },
            "BotList": {
                "w": "https://botlist.me/bots/732917262297595925/vote",
                "v": None,
            },
            "StellarBL": {
                "w": "https://stellarbotlist.com/bot/732917262297595925/vote",
                "v": None,
            },
            "InfinityBL": {
                "w": "https://infinitybotlist.com/bots/732917262297595925/vote",
                "v": None,
            },
            "Astralist": {
                "w": "https://astralist.tk/bot/732917262297595925/vote",
                "v": None,
            },
        }
        async with aiohttp.ClientSession() as cs:
            ...
            try:
                async with cs.get(
                    url=f"https://top.gg/api/bots/{self.bot.user.id}/check?userId={ctx.author.id}",
                    headers=header,
                    timeout=5,
                ) as data:
            ...
            try:
                async with cs.get(
                    url=f"https://api.voidbots.net/bot/voted/{self.bot.user.id}/{ctx.author.id}",
                    headers=header,
                    timeout=5,
                ) as data:
                    json = await data.json()
                    if json.get("voted") is True:
                        bl["VoidBots"]["v"] = True
                        bl["VoidBots"]["nextvote"] = round(
                            datetime.datetime.now().timestamp()
                        ) + round(json["nextVote"]["ms"] / 1000)
            except:
                pass
            header["Authorization"] = os.getenv("listcord_token")
            try:
                async with cs.get(
                    url=f"https://listcord.gg/api/bot/{self.bot.user.id}/voted",
                    headers=header,
                    params={"user_id": f"{ctx.author.id}"},
                    timeout=5,
                ) as data:
                    json = await data.json()
                    if json.get("voted") is True:
            ...
            try:
                async with cs.get(
                    url=f"https://blist.xyz/api/v2/bot/{self.bot.user.id}/votes",
                    headers=header,
                    timeout=5,
                ) as data:
                    json = await data.json()
                    json["votes"].reverse()
                    if (
                        str(ctx.author.id) in str(json["votes"])
                        and datetime.datetime.fromisoformat(
                            [
                                vote
                                for vote in json["votes"]
                                if vote["user"] == str(ctx.author.id)
                            ][0]["time"][:-1]
                            + "+00:00"
                        ).timestamp()
                        + 60 ** 2 * 12
        ...
        embed = discord.Embed(
            title="Vote for Infinity",
            description=f"{voted}\n\n{f'Yet to vote in {len(v.children)} sites. <:down:876079229744316456>' if len(v.children) > 0 else ''}",
    ...
    async def topggvotes(self, ctx):
        ...
        async with aiohttp.ClientSession() as cs:
            header = {"Authorization": os.getenv("topgg_token")}
            async with cs.get(
                url=f"https://top.gg/api/bots/{self.bot.user.id}/votes", headers=header
            ) as data:
                json = await data.json()
        await cs.close()
        text = "".join(
    ...
    async def blistvotes(self, ctx):
        ...
        await ctx.send(
            f"{[vote for vote in json['votes'] if vote['user'] == str(ctx.author.id)]} {str(ctx.author.id) in str(json['votes'])}"
        )
        await ctx.send(
            f"{discord.utils.format_dt(datetime.datetime.fromisoformat([vote for vote in json['votes'] if vote['user'] == str(ctx.author.id)][0]['time'][:-1]+'+00:00'))}"
        )
    ...
    @commands.group(
        name="managers", aliases=["staff", "manager"], invoke_without_command=True
    )
    async def managers(self, ctx):
        """Shows the managers of the bot."""
        managers = self.bot.managers
        embed = discord.Embed(
            title="Infinity Managers",
            description="\n".join([f"<@{manag}>" for manag in managers]),
            color=discord.Color.random(),
        )
    ...
    async def suggestionaccept(self, ctx, *, text: str = None):
        ...
        if ref is None:
            await ctx.send(
                f"{ctx.author.mention} Eh you gotta reply to the suggestion you wanna accept!",
                delete_after=5,
            )
        else:
