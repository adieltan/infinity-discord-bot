...
class UtilityCog(commands.Cog, name="Utility"):
    ...
    async def time(self, ctx, *, expression: str):
        ...
        text = "\n".join(
            f"`{discord.utils.format_dt(time[0], style=style)}`\u2800\u2800{discord.utils.format_dt(time[0], style=style)}"
            for style in styles
        )
    ...
    async def weather(self, ctx, *, city_name: str):
        ...
        async with aiohttp.ClientSession() as cs:
            async with cs.get(
                url=f"http://api.openweathermap.org/data/2.5/weather?appid=e18ed14fc4fe401e9b9133ef3e1ccee6&units=metric&q={city_name}"
            ) as w:
                x = await w.json()
        if x["cod"] != "404":
            y = x["main"]
            ...
            sys = x["sys"]
            embed = discord.Embed(
                title=f"Weather in {x['name']}",
                description=f"**Country**: {sys['country']}\n**Coordinates:** {x['coord']['lon']}, {x['coord']['lat']}",
                color=discord.Colour.random(),
                timestamp=datetime.datetime.fromtimestamp(
                    x["dt"], pytz.timezone("UTC")
                ),
            )
            embed.set_footer(text="Updated on")
            embed.add_field(
                name="Description",
                value=f"{z[0].get('description').title()}",
                inline=False,
            )
            embed.add_field(name="Temperature", value=f"{y.get('temp')}°C", inline=True)
            embed.add_field(
                name="Humidity(%)", value=f"{y.get('humidity')}%", inline=True
            )
            embed.add_field(
                name="Atmospheric Pressure(hPa)",
                value=f"{y.get('pressure')} hPa",
                inline=False,
            )
            embed.add_field(
                name="Wind",
                value=f"Speed: {x.get('wind', {}).get('speed')} m/s\nHeading: {x.get('wind', {}).get('deg')}°\nGust: {x.get('wind', {}).get('gust')} m/s",
                inline=False,
            )
            embed.set_thumbnail(
                url=f"http://openweathermap.org/img/wn/{z[0]['icon']}@2x.png"
            )
        else:
            embed = discord.Embed(
                title="City Not Found",
                description=f"{city_name} is not found.",
    ...
    async def poll(self, ctx, *, question):
        ...
        embed = discord.Embed(
            title="Simple Poll",
            description=f"{ctx.author.mention} asks: **{question}**",
    ...
    async def define(self, ctx, *, phrase: str):
        ...
        async with aiohttp.ClientSession() as cs:
            ...
            async with cs.get(
                url=f"https://owlbot.info/api/v4/dictionary/{phrase}", headers=header
            ) as data:
        ...
        if json.get("detail") is not None:
            ...
            embed = discord.Embed(
                title=json.get("word"),
                description=f"*Pronounciation:* {json.get('pronounciation')}",
                color=discord.Color.random(),
            )
            for defi in json.get("definitions"):
                ...
                if defi.get("example") is not None:
                    ...
                    text += f"\nEg: \u2800\u2800\u2800\u2800\u2800{example}"
                embed.add_field(
                    name=f"{defi.get('emoji')} {defi.get('type')}",
                    value=text,
                    inline=False,
                )
            await ctx.reply(embed=embed, mention_author=False)
    ...
    async def qr(self, ctx, *, text: str = None):
        ...
        embed = discord.Embed(
            title="Qr Generator", description=f"{text}", color=discord.Color.random()
        )
    ...
    async def morse(self, ctx, *, text: str):
        ...
        embed = discord.Embed(
            title=text, description=cipher, color=discord.Color.random()
        )
        ...
        for letter in morsecode:
            ...
            # in case of space
                ...
                if i == 2:
                    ...
                    citext = ""
        embed = discord.Embed(
            title="Morse Decoder", description=decipher, color=discord.Color.random()
        )
        embed.timestamp = discord.utils.utcnow()
        await ctx.reply(embed=embed, mention_author=True)
    ...
    async def youtubestats(self, ctx, channelid: str):
        async with ctx.typing():
            async with aiohttp.ClientSession() as cs:
                async with cs.get(
                    url=f"https://www.googleapis.com/youtube/v3/channels?part=statistics&id={channelid}&key={os.getenv('googleapi')}"
                ) as data:
                    json = await data.json()
            ...
            try:
                ...
                embed.add_field(
                    name="Subscribers",
                    value=f"{format(int(stats['subscriberCount']), ',')}",
                )
                embed.add_field(
                    name="View Count", value=f"{format(int(stats['viewCount']), ',')}"
                )
                embed.add_field(
                    name="Video Count", value=f"{format(int(stats['videoCount']), ',')}"
                )
    ...
    async def amari_reward(self, ctx, target: Member = None):
        ...
        async with aiohttp.ClientSession(
            ...
            async with cs.get(
                url=f"https://amaribot.com/api/v1/guild/rewards/{ctx.guild.id}",
                headers={"Authorization": os.getenv("amari")},
            ) as data:
                json = await data.json()
            async with cs.get(
                url=f"https://amaribot.com/api/v1/guild/{ctx.guild.id}/member/{target.id}",
                headers={"Authorization": os.getenv("amari")},
            ) as data:
        ...
        for r in json["data"]:
            ...
            if role > ctx.me.top_role:
                return await ctx.reply(
                    f"Bot highest role below {role.mention}.",
        ...
        await ctx.reply(
            ...
            .add_field(
                name="Roles Removed",
                value="\u200b" + "\n".join(r.mention for r in remove),
            )
            .set_thumbnail(url="https://amaribot.com/images/Logo.png")
        )
    ...
    async def space(self, ctx):
        ...
        async with aiohttp.ClientSession(
            ...
            async with cs.get(url="http://api.open-notify.org/iss-now.json") as data:
                iss = await data.json()
            async with cs.get(
                url=f"https://api.nasa.gov/planetary/apod?api_key={os.getenv('nasa')}"
            ) as data:
                img = await data.json()
        astraunauts = ""
        ...
        e = discord.Embed(
            title="Space",
            description=f"**People in Space Right Now:**\nNumber of people: {json.get('number')}\n{astraunauts}\n\n**Astronomy Picture of the Day**:\n{img.get('explanation', '')}\n{img.get('title', '')} [<:down:876079229744316456>]({img.get('hdurl')})",
            color=discord.Color.random(),
        )
        try:
    ...
    async def attachments(self, ctx, channelid_or_messageid: str = None):
        ...
        if channelid_or_messageid is None and ref is not None:
            ...
            await ctx.reply(
                "You have to reply to or provide the message id to the message."
            )
