...
class Context(commands.Context):
    async def tick(self, value=True):
        ...
        emoji = (
            "<a:verified:876075132114829342>"
            if value
            else "<:exclamation:876077084986966016>"
        )
        try:
    ...
    async def reply(
        self, content: typing.Optional[str] = None, **kwargs: typing.Any
    ) -> discord.Message:
        try:
...
class Infinity(commands.Bot):
    def __init__(self):
        super().__init__(
            ...
            activity=discord.Activity(
                type=discord.ActivityType.watching, name="the startup process."
            ),
        )
    ...
    async def on_ready(self):
        ...
        self.db = motor.motor_asyncio.AsyncIOMotorClient(
            str(os.getenv("mongo_server")), ssl_cert_reqs=ssl.CERT_NONE
        ).infinity
        self.snipedb = dict({})
        ...
        for filename in os.listdir(
            os.path.dirname(os.path.abspath(__file__)) + slash + "cogs"
        ):
    ...
    async def on_error(self, event, *args, **kwargs):
        ...
        await self.errors.send(
            f"{discord.utils.format_dt(discord.utils.utcnow(), style='t')} Ignoring exception in {event}",
            file=discord.File(buffer, filename="traceback.txt"),
        )
...
def blacklisted(ctx) -> bool:
    return (
        ctx.author.id not in ctx.bot.bled
...
async def performance():
    ...
    embed = discord.Embed(
        ...
        description=f"<:ping:901051623416152095> {pingvalue}ms\n<:cpu:901051865960181770> {cpuvalue}%\n<:memory:901049486242091049> {memoryvalue}%",
...
async def delete_snipecache():
    for i in bot.snipedb.copy():
        if round(bot.snipedb[i].created_at.timestamp() + 60) < round(
