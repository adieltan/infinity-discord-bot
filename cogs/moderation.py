...
class ModerationCog(commands.Cog, name="Moderation"):
            ...
            if (
                ...
                try:
                    await ctx.guild.ban(
                        ...
                        reason=f"Banned by {ctx.author.name} ({ctx.author.id}) for {reason}",
                    )
                    await ctx.reply(
                        f"**{member}** was ***BANNED***\nReason: __{reason}__",
    ...
    async def unban(self, ctx, member: discord.User, *, reason=None):
        ...
        await ctx.guild.unban(
            member, reason=f"Unbanned by {ctx.author.name} for {reason}"
        )
        await ctx.reply(
            f"**{member}** was ***UNBANNED***\nReason: __{reason}__",
            mention_author=False,
        )
    ...
    async def setnick(self, ctx, member: discord.Member, *, nickname: str):
        ...
        await ctx.reply(
            embed=discord.Embed(
                ...
                description=f"{member.mention}'s nickname set to {nickname}",
                color=member.color,
            )
        )
    ...
    async def slowmode(self, ctx, seconds: int = 0):
        ...
        await ctx.reply(
            f"The slowmode delay for this channel is now {seconds} seconds!"
        )
    ...
    async def role_colour(self, ctx, role: Role, colour_hex: str = None):
        ...
        await role.edit(
            ...
            reason="Changed by {ctx.author.name}.",
        )
        embed = discord.Embed(
            title="Role Colour",
            description=f"{role.mention} colour edit.",
    ...
    async def delete(self, ctx, role: Role, *, reason: str = None):
        ...
        try:
            ...
            embed = discord.Embed(
                title="Role deletion",
                description=f"{role.name} deleted.",
                color=discord.Color.random(),
            )
            embed.timestamp = discord.utils.utcnow()
            await ctx.reply(embed=embed, mention_author=False)
    ...
    async def create(self, ctx, rolename: str):
        ...
        try:
            ...
            embed = discord.Embed(
                title="Role creation",
                description=f"{role.mention} created.",
                color=role.color,
            )
            embed.timestamp = discord.utils.utcnow()
            await ctx.reply(embed=embed, mention_author=False)
    ...
    async def info(self, ctx, *, role: Role = None):
        ...
        embed = discord.Embed(
            title="Role Info",
            description=f"{role.mention} Pos: {role.position} `{role.id}`\nMembers: {len(role.members)}",
            color=role.color,
        )
        embed.add_field(
            name="Permissions",
    ...
    async def list(self, ctx):
        ...
        pagess = [
            discord.Embed(
                ...
                description="\n".join([f"{r.mention} `{r.id}`" for r in page]),
                color=discord.Color.random(),
            )
            .set_footer(text=f"{pages.index(page) + 1} / {len(pages)} pages")
    ...
    async def clear(self, ctx, *, member: Member):
        ...
        await member.remove_roles(
            *tuple(roles),
            reason=f"`role clear` command by {ctx.author.name} ({ctx.author.id})",
        )
        embed = discord.Embed(
            title="User role remove all",
            description=f"Removed **{len(roles)}** roles\n{' '.join([r.mention for r in roles])}",
    ...
    async def roleall(self, ctx, *, role: Role):
        ...
        for m in members:
            if role not in m.roles:
                try:
                    await m.add_roles(
                        role,
                        reason=f"`role all` command by {ctx.author.name} ({ctx.author.id})",
                    )
                except:
                    pass
                else:
        ...
        await ctx.reply(
            embed=discord.Embed(
                title="Role all command",
                description=f"Sucessfully added {role.mention} to **{success}** members out of {len(members)} members.",
    ...
    async def rolebots(self, ctx, *, role: Role):
        ...
        members = ctx.guild.members
        await ctx.reply(
            f"Trying to add `{role.name}` to bots in {len(members)} members."
        )
        success = 0
        for m in members:
            if m.bot is True and role not in m.roles:
                try:
                    await m.add_roles(
                        role,
                        reason=f"`role bots` command by {ctx.author.name} ({ctx.author.id})",
                    )
                except:
                    pass
                else:
        ...
        await ctx.reply(
            embed=discord.Embed(
                title="Role bots command",
                description=f"Sucessfully added {role.mention} to **{success}** bots out of {len(members)} members.",
                color=discord.Color.green(),
            )
        )
    ...
    async def rolehumans(self, ctx, *, role: Role):
        ...
        members = ctx.guild.members
        await ctx.reply(
            f"Trying to add `{role.name}` to humans in {len(members)} members."
        )
        success = 0
        for m in members:
            if m.bot is False and role not in m.roles:
                try:
                    await m.add_roles(
                        role,
                        reason=f"`role humans` command by {ctx.author.name} ({ctx.author.id})",
                    )
                except:
                    pass
                else:
        ...
        await ctx.reply(
            embed=discord.Embed(
                title="Role humans command",
                description=f"Sucessfully added {role.mention} to **{success}** humans out of {len(members)} members.",
                color=discord.Color.green(),
            )
        )
    ...
    async def rolerall(self, ctx, *, role: Role):
            ...
            if role in m.roles:
                try:
                    await m.remove_roles(
                        role,
                        reason=f"`role rall` command by {ctx.author.name} ({ctx.author.id})",
                    )
                except:
                    pass
                else:
        ...
        await ctx.reply(
            embed=discord.Embed(
                title="Role rall command",
                description=f"Sucessfully removed {role.mention} from **{success}** members out of {len(members)} members.",
                color=discord.Color.red(),
            )
        )
    ...
    async def rolerbots(self, ctx, *, role: Role):
        ...
        await ctx.reply(
            f"Trying to remove `{role.name}` from bots in {len(members)} members."
        )
        success = 0
        for m in members:
            if m.bot is True and role in m.roles:
                try:
                    await m.remove_roles(
                        role,
                        reason=f"`role rbots` command by {ctx.author.name} ({ctx.author.id})",
                    )
                except:
                    pass
                else:
        ...
        await ctx.reply(
            embed=discord.Embed(
                title="Role rbots command",
                description=f"Sucessfully removed {role.mention} from **{success}** bots out of {len(members)} members.",
                color=discord.Color.red(),
            )
        )
    ...
    async def rolerhumans(self, ctx, *, role: Role):
        ...
        members = role.members
        await ctx.reply(
            f"Trying to remove `{role.name}` from humans in {len(members)} members."
        )
        success = 0
        for m in members:
            if m.bot is False and role in m.roles:
                try:
                    await m.remove_roles(
                        role,
                        reason=f"`role rhumans` command by {ctx.author.name} ({ctx.author.id})",
                    )
                except:
                    pass
                else:
        ...
        await ctx.reply(
            embed=discord.Embed(
                title="Role rhumans command",
                description=f"Sucessfully removed {role.mention} from **{success}** humans out of {len(members)} members.",
                color=discord.Color.green(),
            )
        )
    ...
    async def userinfo(
        ...
        if type(member) == discord.Member:
            ...
            embed = discord.Embed(
                title="User Info",
                description=f"{member.mention} {member} [Avatar]({member.display_avatar})\n{status}\n",
                color=member.color,
