...
class ServerCog(commands.Cog, name="Server"):
    ...
    async def prefix(self, ctx, prefix: str = None):
        ...
        if not prefix:
            ...
            await ctx.reply(
                f"The prefix for {ctx.guild.name} is `{results.get('prefix', '=')}`",
    ...
    async def serverinfo(self, ctx, *, guild_id: int = None):
        ...
        info = [
            f"<a:verified:876075132114829342> {feature.replace('_', ' ').lower().title()}"
        ...
        e.add_field(
            name="Channels",
            value=f"<:text_channel:874547994584829982> {len(channels)}\n<:voice_channel:874548222708826153> {len(vc)}",
        )
        ...
        e.add_field(
            name="Roles",
            value=f"<:Role_Icon:882098706437001276> {len(guild.roles)} roles",
        )
    ...
    async def ll(self, ctx):
        ...
        await ctx.reply(
            "Leave Leaderboard",
            file=discord.File(buffer, filename=f"{ctx.guild.id}_leaveleaderboard.txt"),
        )
    ...
    async def add_ar(self, ctx, *, trigger: str):
        ...
        if len(list(ar.keys())) >= 10:
            return await ctx.reply(
                "This guild has reached the maximum number of autoresponse which is 10."
            )
        await ctx.reply(
            embed=discord.Embed(
                title="Autoresponse",
                description=f"Type the response you want for `{trigger}`.",
            )
        )
        try:
        ...
        if len(response.clean_content) >= 500:
            ...
            await ctx.reply(
                embed=discord.Embed(
                    title="New Autoresponse",
                    description=f"Response for `{trigger}` set to `{response.clean_content}`",
                )
            )
    ...
    async def remove_ar(self, ctx, *, trigger: str):
        ...
        try:
            ...
            await ctx.reply(
                embed=discord.Embed(
                    title="Removed Autoresponse",
                    description=f"Removed response for `{trigger}`.",
                )
            )
            self.bot.ar[f"{ctx.guild.id}"] = results["autoresponse"]
        except:
    ...
    async def list_ar(self, ctx):
        ...
        await ctx.reply(
            embed=discord.Embed(
                title=f"{ctx.guild.name}'s Autoresponses", description=f"{text}"
            )
        )
    ...
    async def on_message(self, message: discord.Message):
        ...
        try:
            ...
            if any(key in clean for key in keys):
                text = "\n".join(
                    self.bot.ar.get(f"{message.guild.id}", {})[key]
                    for key in keys
                    if key in clean
                )
                await message.reply(
    ...
    async def roledel(self, ctx, roles: commands.Greedy[Role] = None):
        ...
        embed = discord.Embed(
            title="Role Deletion",
            description=f"Deleting {len(roles)} roles.\n{' '.join([role.mention for role in roles])}\nClick Confirm.",
    ...
    async def catdel(self, ctx, categorys: commands.Greedy[discord.CategoryChannel]):
        ...
        embed = discord.Embed(
            ...
            description=f"Deleting {len(categorys)} categories.\n{' '.join([cate.mention for cate in categorys])}\nClick Confirm.",
            color=discord.Color.brand_red(),
        )
        v.msg = await ctx.reply(embed=embed, view=v)
        msgv = discord.ui.View.from_message(v.msg)
    ...
    async def chandel(
        ...
        embed = discord.Embed(
            ...
            description=f"Deleting {len(channels)} channels.\n{' '.join([chan.mention for chan in channels])}\nClick Confirm.",
            color=discord.Color.brand_red(),
        )
        v.msg = await ctx.reply(embed=embed, view=v)
        msgv = discord.ui.View.from_message(v.msg)
    ...
    async def purgeallchannel(self, ctx):
        ...
        v = Confirm(ctx)
        embed = discord.Embed(
            title="Guild Channel Deletion",
            description=f"Deleting {len(ctx.guild.channels)} channels.\nClick Confirm.",
            color=discord.Color.brand_red(),
        )
        v.msg = await ctx.reply(embed=embed, view=v)
        msgv = discord.ui.View.from_message(v.msg)
    ...
    async def hide(
        ...
        e = discord.Embed(
            title="Channel Hide",
            description=f"Hidden {len(channels)} channel(s):\n{c}\nfor {target.mention}",
            color=discord.Color.random(),
        )
        await ctx.reply(embed=e)
    ...
    async def unhide(
        ...
        e = discord.Embed(
            title="Channel Unide",
            description=f"Unhidden {len(channels)} channel(s):\n{c}\nfor {target.mention}",
            color=discord.Color.random(),
        )
        await ctx.reply(embed=e)
    ...
    async def lock(
        ...
        e = discord.Embed(
            title="Channel Lock",
            description=f"Locked {len(channels)} channel(s):\n{c}\nfor {target.mention}",
            color=discord.Color.random(),
        )
        await ctx.reply(embed=e)
    ...
    async def unlock(
        ...
        e = discord.Embed(
            ...
            description=f"Unlocked {len(channels)} channel(s):\n{c}\nfor {target.mention}",
            color=discord.Color.random(),
        )
        await ctx.reply(embed=e)
    ...
    async def export(
        ...
        ctx,
        destination,
        first_message_id: typing.Optional[int] = None,
        limit: typing.Optional[int] = 100,
        mode: typing.Literal["clean"] = None,
    ):
        ...
        if first_message_id:
            ...
            first_message_id = await ctx.channel.fetch_message(
        ...
        if channel_perms.get("administrator") is not True:
            raise commands.MissingPermissions(missing_perms=["administrator"])
        bot_perms_in_channel = dict(
            iter(
                destination_channel.permissions_for(
                    await destination_channel.guild.fetch_member(self.bot.user.id)
                )
            )
        )
        ...
        if any(
            w for w in presentwebhooks if w.name == "Export"
        ):  # len(presentwebhooks) > 0:
        ...
        async for m in ctx.channel.history(
            limit=limit, after=first_message_id, oldest_first=True
        ):
            if m == ctx.message:
                break
            try:
                ...
                msg = await last_webhookmsg.edit(
                    content=content,
                    embeds=last_message.embeds + m.embeds,
                    allowed_mentions=discord.AllowedMentions.none(),
                )
                last_webhookmsg = msg
                m.content = content
                ...
                if len(content) > 2000:
                    ...
                    msg = await webhook.send(
                        file=discord.File(buffer, filename="export.txt"),
                        wait=True,
        ...
        *,
        ...
        if channel is not None:
            ...
            e = discord.Embed(
                ...
                description=f"Starting interactive session to rename all channels.",
            ...
            for channel in ctx.guild.channels:
                ...
                try:
                    response = await self.bot.wait_for(
                        "message", check=msg_check, timeout=180
                    )
                    if "skip" in response.content.lower():
                        pass
                    elif (
                        "end" in response.content.lower()
                        or "quit" in response.content.lower()
                    ):
                        break
                    else:
            ...
            result = await ctx.reply(
                embed=discord.Embed(
                    title="Channel Rename",
                    description="\n".join(
                        f"{chan[0].mention} {chan[1]} -> {chan[2]}" for chan in edits
                    ),
    ...
    async def overwrites_remove(
        ...
        await ctx.reply(
            f"Permission for {channel.mention} has been synced with {channel.category.mention}."
        )
