...
class UsersCog(commands.Cog, name="User"):
    ...
    async def selfharm(self, ctx, victim: discord.Member = None):
        ...
        if not victim:
            victim = ctx.author
        embed = discord.Embed(
            title="Suicide & Selfharm Prevention",
            url="https://www.who.int/health-topics/suicide",
            description="You are not alone. Everyone is special in their own ways and thats why you shouldn't give up.",
            color=discord.Color.blurple(),
        )
        embed.set_author(name=ctx.author.name, icon_url=ctx.author.avatar)
        embed.add_field(
            name="Get some help today.",
            value="[Suicide prevention](https://www.who.int/health-topics/suicide)\n[Contact Numbers](https://www.opencounseling.com/suicide-hotlines)\n[Crisis Lines](https://en.wikipedia.org/wiki/List_of_suicide_crisis_lines)",
        )
        embed.set_thumbnail(
            url="https://www.nursingcenter.com/getattachment/NCBlog/September-2016-(1)/World-Suicide-Prevention-Day/2016_wspd_ribbon_250X250.png.aspx?width=200&height=200"
        )
        embed.set_image(
            url="https://sm.mashable.com/mashable_me/photo/default/gettyimages-6130324100_675p.jpg"
        )
        try:
    ...
    async def profile(self, ctx, member: discord.User = None):
        ...
        embed = discord.Embed(
            title="Profile", description="", color=discord.Color.random()
        )
        ...
        if results.get("country"):
            ...
            embed.add_field(
                name="Location",
                value=f":flag_{fuz[0].alpha_2.lower()}: {fuz[0].name}" + "\n",
            )
        if bd:
        ...
        if results.get("manager"):
            embed.description += (
                "<a:infinity:874548940610097163> **Infinity Managers**\n"
            )
    ...
    async def setbiography(self, ctx, *, bio: str):
        ...
        await Database.edit_user(
            self, ctx.author.id, {"bio": discord.utils.remove_markdown(bio)}
        )
